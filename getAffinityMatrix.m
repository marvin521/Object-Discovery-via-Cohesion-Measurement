function [A,A1]=getAffinityMatrix(I,epsilon,win_size)
  ...
  for j=1+win_size:w-win_size
    for i=win_size+1:h-win_size
      ...
      win_mu=mean(winI,1)';
      win_var=inv(winI'*winI/neb_size-win_mu*win_mu' +epsilon/neb_size*eye(c));  % the formula of corvariance
  ...
  [ir,ic,iv]=find(A);
  sumA=sum(A,2);
  normDvision=sqrt(sumA(ir).*sum(ic));
  newAv=iv./normDvision;
  %A(ir,ic)=newAv;
  A=sparse(ir,ic,newAv,img_size,img_size);
